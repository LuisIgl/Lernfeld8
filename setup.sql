...
CREATE TABLE `quiz_game`.`Account` (
...
CREATE TABLE `quiz_game`.`Kategorie` (
    `KategorieNr` INT NOT NULL,
    `Kategorie` TEXT NOT NULL,
    PRIMARY KEY (`KategorieNr`)
) ENGINE = InnoDB;

CREATE TABLE `quiz_game`.`Fragen` (
...
CREATE TABLE `quiz_game`.`Antworten` (
...
INSERT INTO Account (AccountNr, Username, Password) VALUES (001, 'admin', 'admin');
